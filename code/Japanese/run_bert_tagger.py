...
lst = ['O','助動詞', '記号', '名詞', '接頭詞', '形容詞', '連体詞', '感動詞', 'その他', '接続詞', '副詞', '助詞', '動詞', 'フィラー']
label_map = {lb: i for i, lb in enumerate(lst)}
with open("/content/mydrive/COMP551A4/glyce/japanese/POS/dev.char.bmes", "r") as f:
...
dev_labels = []
for i in dev.split():
    dev_labels.append(label_map[i])
with open("/content/mydrive/COMP551A4/glyce/japanese/POS/test.char.bmes", "r") as v:
    reader = csv.reader(v, delimiter="\t", quotechar=None)
    lab = []
    for line in reader:
      lab.append(line[1])  
test = ' '.join(lines)
...
def load_data(config):
    ...
    if config.data_sign == "msra_ner":
        data_processor = MsraNERProcessor()
    elif config.data_sign == "Japanese_pos":
        data_processor = JapanesePOSProcessor()
    ...
    # return dev_dataloader, label_list 
    ...
    for idx in range(int(config.num_train_epochs)):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if nb_tr_steps % config.checkpoint == 0:
                ...
                eval_checkpoint(model, dev_dataloader, config, device, n_gpu, label_list,dev_labels, eval_sign="dev")
                print("......"*10)
                ...
                eval_checkpoint(model, test_dataloader, config, device, n_gpu, label_list,test_labels, eval_sign="test")
                print("......"*10)
...
def eval_checkpoint(model_object, eval_dataloader, config, \
    device, n_gpu, label_list,labels, eval_sign="dev"):
    ...
    for input_ids, input_mask, segment_ids, label_ids in eval_dataloader:
        ...
        mask_lst += input_mask 
        eval_steps += 1   
        # print(len(pred_lst[index]))
        f1 = f1+f1_score(pred_lst[index], labels[index*len(pred_lst[index]):(index+1)*len(pred_lst[index])],average='weighted', labels=np.unique(pred_lst[index]))
        pre =pre+ precision_score(pred_lst[index], labels[index*len(pred_lst[index]):(index+1)*len(pred_lst[index])], average='weighted', labels=np.unique(pred_lst[index]))
        rec =rec+ recall_score(pred_lst[index], labels[index*len(pred_lst[index]):(index+1)*len(pred_lst[index])], average='weighted', labels=np.unique(pred_lst[index]))
        acc = acc +accuracy_score(pred_lst[index], labels[index*len(pred_lst[index]):(index+1)*len(pred_lst[index])], average='weighted', labels=np.unique(pred_lst[index]))
        # print(pred_lst[0]) 
        # print("********************hh********")
        # print(labels[0:len(pred_lst[0])])
        # print("f1_score: ",f1_score(pred_lst[index], labels[index*len(pred_lst[index]):(index+1)*len(pred_lst[index])], average="macro"))
        # print("precision: ",precision_score(pred_lst[index], labels[index*len(pred_lst[index]):(index+1)*len(pred_lst[index]), average="macro"))
        # print("recall: ",recall_score(pred_lst, labels, average="macro"))   
        index = index+1 
    # print("\nfor test:\n")
    # print(f1_score(pred_lst, test_labels, average="macro"))
    # print(precision_score(pred_lst, test_labels, average="macro"))
    # print(recall_score(pred_lst, dev_labels, average="macro"))    
    # eval_accuracy, eval_precision, eval_recall, eval_f1 = compute_performance(pred_lst, gold_lst, mask_lst, label_list, dims=2)  
    print("********************hh********")
    # print(labels[0:len(pred_lst[0])])
    print("f1_score: ",f1/(index+1))
    print("precision: ",pre/(index+1))
    print("recall: ",rec/(index+1))  
    print("accuray: ", acc/(index+1))
