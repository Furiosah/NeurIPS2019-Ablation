...
class JapanesePOSProcessor(DataProcessor):
    def get_train_examples(self, data_dir):
        examples= self._create_examples(self._read_tsv(os.path.join(data_dir, "train.char.bmes")), "train")
        return examples

    def get_test_examples(self, data_dir): 
        examples= self._create_examples(self._read_tsv(os.path.join(data_dir, "test.char.bmes")), "test")
        return examples


    def get_dev_examples(self, data_dir):
        # print()
        examples= self._create_examples(self._read_tsv("/content/mydrive/COMP551A4/glyce/japanese/POS/dev.char.bmes"), "dev")
        return examples


    def get_labels(self):
        return ['O','助動詞', '記号', '名詞', '接頭詞', '形容詞', '連体詞', '感動詞', 'その他', '接続詞', '副詞', '助詞', '動詞', 'フィラー']
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            label = label.split(" ")
            # print(label)
            guid = "{}_{}".format("Japanesepos", str(i)) 
